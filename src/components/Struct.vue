...
export default {
    ...
    methods: {
        load() {
            ...
            let val;
            if (is.type(this.value, 'Function')||is.type(this.defaultValue, 'Function')) {
                val = String(this.value ? this.value : this.defaultValue);
                console.log(val);
            } else {
                val = toJSON(this.value ? deepParseFn(deepCopy(this.value)) : this.defaultValue);
                console.log(val);
            }
            this.oldVal = val;
            this.$nextTick(() => {
